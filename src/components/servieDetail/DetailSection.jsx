...
import PackageCarousel from "../Home/PackageCarousel";
import LanguageClassCarousel from "../Home/LanguageClassCarousel";
...
function DetailSection({ id }) {
  ...
  useEffect(() => {
    ...
    const fetchService = async () => {
      ...
      setType(response.data.serviceCategory);
      setLoading(false);
    };
    fetchService();
  }, [id]);
  ...
  return (
    ...
    <div>
      ...
      {type === "services" && <ServicesSection id={id} />}
      {type === "language-class" && <LanguageClassCarousel id={id} />}
      {type === "blog" && <Blog id={id} />}
      {type === "packages" && <PackageCarousel id={id} />}
    </div>
  );
}
